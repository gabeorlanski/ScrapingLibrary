...
if r'\tests' in os.getcwd() or r'\src' in os.getcwd():
...
class Scraper:
    ...
    def run(self, links=None):
        ...
        r = mp.JoinableQueue()
        workers = self._cores
        consumers = [InstructionWorker(q, r, self._instructions) for i in range(workers)]
        for c in consumers:
            c.start()

        for i in range(len(page_data)):
            k = page_data[i]["key"]
            d = page_data[i]['response'].decode()
            q.put((k, d))
...
if __name__ == "__main__":
    s = Scraper(cores=1)
    links = [{"headers": {}, "dictKey": "random1", "url": "https://en.wikipedia.org/wiki/Siah_Bisheh,_Chalus"},
        ...
        {"headers": {}, "dictKey": "random5", "url": "https://en.wikipedia.org/wiki/2017_BWF_World_Championships_%E2%80%93_Women%27s_singles"}]

    instructions = [
        generate_instruction_dict("//h1[@class='firstHeading']", "title", text=True),
